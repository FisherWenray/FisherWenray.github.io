...
## 2️⃣ 多平台文章同步架构 (本地 -> 网站/公众号)

```text
┌──────────────────────────────────────┐
│           本地创作环境 (Markdown)      │
│  articles/article-XXX.md             │
└──────────────────┬───────────────────┘
                   │
         ┌─────────┴─────────┐
         ↓                   ↓
┌─────────────────┐ ┌───────────────────────────┐
│  GitHub Pages   │ │   微信公众号同步工作流       │
│  (标准 HTML)     │ │   (publish_to_wechat.py)  │
└─────────────────┘ └─────────────┬─────────────┘
                                  │
                          1. 读取 Markdown 到剪贴板
                          2. 打开 MDNice 并应用专属主题 (wechat_theme.css)
                          3. 粘贴后，将内联样式的富文本复制至微信公众号后台
```
